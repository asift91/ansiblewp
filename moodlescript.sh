...
sudo sed -i "s~$CFG->dbhost = 'localhost'~$CFG->dbhost = '${4}'~" /home/azureadmin/ansible_playbook/roles/moodle/templates/config.php  sudo sed -i "s~dbuser = 'db_username'~dbuser = ${5}~" /home/${3}/ansible_playbook/roles/moodle/templates/config.php >> /home/${3}/var.txt
  sudo sed -i "s~dbpass = 'db_password'~dbpass = ${6}~" /home/${3}/ansible_playbook/roles/moodle/templates/config.php >> /home/${3}/var.txt
